(() => {
  ...
  function formatAgo(iso) {
    ...
    if (diffSec < 0) return "";
    if (diffSec < 60) return `${diffSec} 绉掑墠`;
  ...
  async function loadIP() {
    ...
    try {
      ...
      valEl.textContent = data.ip || data.container_ip || "鏈煡";
      const parts = [`鏉ユ簮: ${data.source || "unknown"}`];
      if (data.updated_at) {
        parts.push(`鏇存柊鏃堕棿: ${data.updated_at}`);
      }
      metaEl.textContent = parts.join(" 路 ");
    } catch (e) {
      valEl.textContent = "鏈幏鍙?;
  ...
  async function saveConfig() {
    ...
    try {
      ...
      status.textContent = "淇濆瓨鎴愬姛";
      status.className = "text-sm text-green-600 mt-2";
    } catch (e) {
      status.textContent = "淇濆瓨澶辫触: " + e.toString();
      status.className = "text-sm text-red-600 mt-2";
    }
  }
