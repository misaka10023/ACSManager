...
- 任务列表支持为每个容器配置 `manual` / `auto_on_start` 触发方式、`once` / `ensure_running` 模式，以及 `screen` / `nohup` / `shell` 运行器。
- 按钮：
  - “重新载入”：`GET /config?reload=true`，重新读盘填充表单。
  - “保存配置”：收集表单并调用 `PUT /config` 覆盖配置文件，结果会提示。
  - “更新并重启”（顶部导航按钮）：`POST /update` 先 `git fetch` 检查是否落后，落后才执行 `git pull` 并自动重启；如出现冲突或未配置上游分支会失败。
